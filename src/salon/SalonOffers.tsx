...
import { Percent, Plus, Pencil, Trash2, X, Tag, Loader2 } from 'lucide-react';
...
export function SalonOffers() {
  ...
  const fetchOffers = async () => {
    try {
      const r = await fetch('/api/salon/offers', { headers: authHeaders() });
  ...
  const del = async (id: number) => {
    ...
    try {
      const r = await fetch(`/api/salon/offers/${id}`, { method: 'DELETE', headers: authHeaders() });
  ...
  const toggleActive = async (o: Offer) => {
    try {
      const r = await fetch(`/api/salon/offers/${o.id}`, {
        method: 'PUT', headers: authHeaders(),
        body: JSON.stringify({ ...o, is_active: !o.is_active }),
      });
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setForm(DF); setEditing(null); setShowForm(true); }} className="inline-flex items-center gap-2 bg-indigo-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-indigo-700 transition-colors shadow-sm">
              ...
              <div className="p-4 space-y-2">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => edit(o)} className="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Edit"><Pencil className="w-4 h-4" /></button>
                    <button onClick={() => del(o.id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={() => { setShowForm(false); setEditing(null); setForm(DF); }}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 space-y-5 relative" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button onClick={save} disabled={saving || !form.title.trim() || !form.discount_percent || !form.valid_from || !form.valid_until} className="px-4 py-2.5 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 rounded-xl transition-colors inline-flex items-center gap-2">
